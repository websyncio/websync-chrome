...
export default class CreateComponentTypeAction implements IEditorPopupAction {
    ...
    name: string;
    actionClass = 'create-component-type';
    typeName: string;
    parentId: string;
    synchronizationService: ISynchronizationService;
    onExecuted: () => void;

    constructor(typeName: string, parentId: string, onExecuted) {
    ...
    execute() {
        this.synchronizationService.createComponentType(this.typeName, this.parentId, null);
        this.onExecuted();
    }
}
